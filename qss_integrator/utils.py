...
def create_qss_solver(gas: ct.Solution, pressure: float, config: Dict[str, Any]) -> QssIntegrator:
    ...
    if QssIntegrator is None or PyQssOde is None:
        raise ImportError("QSS C++ extension not available. Please build the package first.")
...
def integrate_single_step(solver_type: str, config: Dict[str, Any], gas: ct.Solution, 
                         state: np.ndarray, t0: float, t_end: float, pressure: float, 
                         dt: float = None, mxsteps: int = 1000) -> Tuple[np.ndarray, float, float, bool]:
    """
