...
def setup_initial_conditions(gas, fuel_composition, oxidizer_composition, equivalence_ratio, T0, pressure):
    """
    ...
    stoich_air_fuel_ratio = gas.stoich_air_fuel_ratio(fuel_composition, oxidizer_composition)
...
def cantera_integration(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio, T0=1400.0, pressure=101325.0, t_final=0.02, reactor_type='constant_pressure'):
    """
    ...
    if reactor_type == 'constant_pressure':
        ...
        raise ValueError("reactor_type must be 'constant_pressure' or 'constant_volume'")
    ...
    for i in range(n_steps):
        ...
        if (i + 1) % 20 == 0:
            ...
            print(f"  t = {sim.time*1000:5.2f} ms, T = {reactor.T:6.1f} K, "
...
def qss_integration_with_cantera(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio, T0=1400.0, pressure=101325.0, t_final=0.02):
    """
    ...
    class CanteraQSSODE:
        ...
        def __call__(self, t, y, corrector=False):
            ...
            net = wQ - wD
            qdot = -np.dot(net, h_form)  # J/m^3/s  (exothermic release is negative enthalpy change)
    ...
    for i in range(1, n_outputs + 1):
        ...
        if i % 20 == 0:
            ...
            fuel_name = list(fuel_composition.keys())[0]  # Primary fuel
            try:
                fuel_idx = gas.species_index(fuel_name) + 1  # +1 because T is at index 0
                Y_fuel = integrator.y[fuel_idx]
                print(f"  t = {integrator.tn*1000:5.2f} ms, T = {T:6.1f} K, {fuel_name} = {Y_fuel:.6f}")
            except:
...
def compare_integrators(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio=1.0, T0=1400.0, t_final=0.02, pressure=101325.0):
    ...
    try:
        cantera_times, cantera_temps, cantera_species, cantera_cpu_times = cantera_integration(
            mechanism, fuel_composition, oxidizer_composition, equivalence_ratio, T0=T0, t_final=t_final, pressure=pressure)
    except Exception as e:
        print(f"Cantera integration failed: {e}")
        return
    
    try:
        qss_times, qss_temps, qss_species, major_species, qss_cpu_times = qss_integration_with_cantera(
            mechanism, fuel_composition, oxidizer_composition, equivalence_ratio, T0=T0, t_final=t_final, pressure=pressure)
    except Exception as e:
    ...
    for i, species in enumerate(major_species):
        ...
        ax.plot(cantera_times * 1000, cantera_species[species], 'b-', label='Cantera', linewidth=2)
    ...
    axes[1].bar([
        ...
        np.sum(cantera_cpu_times), np.sum(qss_cpu_times)
    ])
    ...
    plt.tight_layout()
    plt.savefig(f'cantera_vs_qss_{fuel_name}_phi{equivalence_ratio:.1f}_cpu_time.png', dpi=300)
    plt.close()
...
def test_methane(pressure=101325.0):
    ...
    mechanism = '/Users/elotech/Downloads/research_code/large_mechanism/ch4_53species.yaml'
    fuel_composition = {'CH4': 1.0}
    oxidizer_composition = {'O2': 1.0, 'N2': 3.76}  # Air composition
    compare_integrators(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio=1.0, T0=1200.0, t_final=0.08, pressure=pressure)
...
def test_hydrogen(pressure=101325.0):
    ...
    compare_integrators(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio=1.0, T0=1000.0, t_final=0.01, pressure=pressure)
...
def test_propane(pressure=101325.0):
    ...
    oxidizer_composition = {'O2': 1.0, 'N2': 3.76}
    compare_integrators(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio=1.0, T0=1300.0, t_final=0.05, pressure=pressure)
...
def test_fuel_blend(pressure=101325.0):
    ...
    oxidizer_composition = {'O2': 1.0, 'N2': 3.76}
    compare_integrators(mechanism, fuel_composition, oxidizer_composition, equivalence_ratio=0.8, T0=1400.0, t_final=0.05, pressure=pressure)
